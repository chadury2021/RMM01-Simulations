...
class CoveredCallAMM(object):
    ...
    def __init__(self, initial_x, K, sigma, tau, fee):
        ...
        self.invariant = 0
        self.reserves_riskless = self.K * \
            norm.cdf(norm.ppf(1-initial_x) - self.sigma*np.sqrt(self.tau))
        self.fee = fee
    ...
    def swapAmountInRisky(self, amount_in):
        ...
        gamma = 1 - self.fee
        new_reserves_riskless = self.getRisklessGivenRisky(
            self.reserves_risky + gamma*amount_in)
        ...
        # Update invariant
        self.invariant = self.reserves_riskless - \
            self.getRisklessGivenRiskyNoInvariant(self.reserves_risky)
    ...
    def swapAmountInRiskless(self, amount_in):
        ...
        assert nonnegative(amount_in)
        gamma = 1 - self.fee
        new_reserves_risky = self.getRiskyGivenRiskless(
            self.reserves_riskless + gamma*amount_in)
